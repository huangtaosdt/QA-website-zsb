...
def followers(username):
    ...
    print(len(follows))
    return render_template('followers.html', user=user, title='粉丝列表',fan_counts=len(follows)-1,
...
def followed_by(username):
    ...
    follows = [{'user': item.followed, 'timestamp': item.timestamp}
               for item in pagination.items]
    return render_template('followers.html', user=user, title="关注列表",fan_counts=len(follows)-1,
